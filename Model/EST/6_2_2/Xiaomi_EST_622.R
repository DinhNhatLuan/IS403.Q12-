# --- Exponential Smoothing (ETS) - Xiaomi_clean.csv ---
library(readr)
library(dplyr)
library(ggplot2)
library(scales)
...
data <- read_csv("Xiaomi_clean.csv")
...
p <- ggplot(plot_df, aes(x = Date)) +
  ...
  labs(
    title = "Xiaomi - Exponential Smoothing (6:2:2)",
...
ggsave("Xiaomi_EST_622.png", p, width = 10, height = 5, dpi = 300)
