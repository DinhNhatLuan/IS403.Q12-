...
# --- ĐỌC DỮ LIỆU ---
data <- read.csv("D:/HK5_2025-2026/IS403-PTDLKD/DoAn/Xiaomi_clean.csv")
...
model <- keras_model_sequential() %>%
  layer_lstm(units = 64, input_shape = c(time_step, 1), return_sequences = FALSE) %>%
  layer_dense(units = 1)
...
ggplot() +
  ...
  labs(
    title = "Dự báo xu hướng giá cổ phiếu(LSTM nối tiếp 30 ngày)",
