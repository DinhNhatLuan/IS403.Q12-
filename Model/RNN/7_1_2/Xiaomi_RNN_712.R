# ==========================================================
# XIAOMI_RNN_712.R
# DỰ BÁO GIÁ ĐÓNG CỬA CỔ PHIẾU XIAOMI BẰNG MÔ HÌNH RNN
# ==========================================================
...
# ==========================================================
# HÀM TẠO SEQUENCE (ĐÃ SỬA LỖI FLOAT -> INT & CAST float32)
# ==========================================================
create_sequences <- function(series, timesteps = 10) {
  ...
  # Ép kiểu float32 để TensorFlow xử lý ổn định
...
model <- keras_model_sequential(list(
  ...
  layer_simple_rnn(units = 64, activation = "tanh"),
  layer_dense(units = 1)
))
...
p <- ggplot() +
  ...
  geom_line(data = forecast_df, aes(x = Date, y = Predicted_Close, linetype = "Forecast Next 30 Days"),
            color = "red", size = 0.8) +
  labs(
    title = sprintf("Xiaomi Closing Price Prediction (RNN, %s → %s)",
...
save_path <- "Xiaomi_RNN_712_R.png"
