...
rm(list = ls())
data_path <- "C:/Users/capta/Downloads/IS403.Q12-/Data/Dataset/Xiaomi_clean.csv"
...
fit <- forecast::auto.arima(train_ts, seasonal = FALSE, stepwise = TRUE, approximation = FALSE, ic = "aicc", method = "ML", allowdrift = TRUE, allowmean = TRUE, trace = TRUE)
cat("\n--- Kết quả mô hình ---\n")
print(summary(fit))
...
p <- ggplot() +
  ...
  # Tiêu đề và trục
  labs(
    title = sprintf("Xiaomi Closing Price (%s → %s)", min(df$Date), forecast_df$Date[nrow(forecast_df)]),
...
# Lưu file hình
save_path <- file.path("C:/Users/capta/Downloads/IS403.Q12-/Model/ARIMA/R/7_1_2/Image/Xiaomi_ARIMA_712_R.png")
